...
// Time delay using busy wait.
// This assumes 80 MHz system clock.
void SysTick_Wait13us(unsigned long delay){
  unsigned long i;
  for(i=0; i<delay; i++){
		SysTick_Wait(1053);
  }
...
void SysTick_Wait10us(unsigned long delay){
  unsigned long i;
  for(i=0; i<delay; i++){
		SysTick_Wait(800);   //10us/12.5ns = 10000/12.5= 800
  }
}

void SysTick_Wait12ms(unsigned long delay){
  unsigned long i;
  for(i=0; i<delay; i++){
		SysTick_Wait(960000); //12ms/12.5ns = 12000000/12.5= 960000
  }
}